...
// CanvasWidget::initTextures()がGL_MAX_ARRAY_TEXTURE_LAYERSを問い合わせて決める
...
// (CanvasWidget::rebuildFilterChain 参照)。この構造上、タイル単位で合成する
...
// これを直接編集し、変更のたびにCanvasWidget::rasterizeTextLayer()がここから
...
public:
    ...
    // 変更通知
    // ------------------------------------------------------------------
    // どの粒度の変更が起きたかを表す。呼び出し側(CanvasWidget)はこれを見て
    ...
    // 変更のたびに呼ばれるコールバック。CanvasWidget側で1箇所だけ購読する想定。
    ...
    // 新規タイルはclearFnで透明クリアする(実際のGL処理は呼び出し側=CanvasWidgetが担当する)。
    ...
    // スタック先頭のエントリ自体を覗き見る(pop はしない)。CanvasWidget::undo()/redo() が
    ...
    // CanvasResizeエントリの適用(CanvasWidget::rebuildCanvasFromSnapshots)はタイルグリッド自体を
