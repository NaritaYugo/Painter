...
// CanvasWidget::beginBulkLayerImport()/endBulkLayerImport()のRAIIラッパー。
...
struct BulkLayerImportGuard {
    explicit BulkLayerImportGuard(CanvasWidget *gl) : gl_(gl) { gl_->beginBulkLayerImport(); }
    ~BulkLayerImportGuard() { gl_->endBulkLayerImport(); }
    CanvasWidget *gl_;
};
}

CanvasSerializer::CanvasSerializer(CanvasWidget *gl)
    : gl_(gl)
{}
...
bool CanvasSerializer::load(const QString &path)
{
    ...
    // (新規タブ等の)CanvasWidgetが持つ現在のサイズとは無関係に、どんなサイズの
    // ファイルでも開けるようにする)
    // ------------------------------------------------------------------
