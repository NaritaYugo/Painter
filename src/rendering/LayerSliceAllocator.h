...
//
// バンクテクスチャの実体(GLuint[])はCanvasWidgetが所有する配列を指しており、このクラスは
...
{
public:
    // banks: CanvasWidgetが所有する GLuint[MAX_TILE_BANKS] への先頭ポインタ(このクラスが
    ...
    // (テクスチャの中身をゼロクリアするのは呼び出し側=CanvasWidgetの責務のまま)
