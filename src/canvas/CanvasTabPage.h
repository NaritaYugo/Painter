...
//
// CanvasWidgetはここが親になる。所属タブはgl->parentWidget()をqobject_castすれば引ける
// (MainWindow側でCanvasWidget->タブの対応表を別に持つ必要はない)。
// ---------------------------------------------------------------------------
...
public:
    ...
    // CanvasWidgetをこのタブに載せる。1タブにつき1回だけ呼ぶ(2回目以降・nullptrは無視)。
    //
    // 重要: ここでは表示は切り替えず、スタートページを出したままにする。
    // このタブが既に画面に出ている状態でCanvasWidgetをカレントにすると、その場で
    // 同期的にshow()→resizeEvent→initializeGL()まで走り、CanvasWidget::initializedが
    // 呼び出し元へ戻る前に発行され切ってしまう。ファイル読み込み/新規キャンバス作成は
    // そのシグナルを待って行うため、接続を張る前に発行されると何も起きない
    // (=初期状態の空キャンバスのまま)。表示への切り替えはshowCanvas()で明示的に行う。
    void setCanvas(CanvasWidget *gl);

    // 実際にキャンバス表示へ切り替える(CanvasWidget::initializedへの接続を済ませてから呼ぶ)。
    void showCanvas();
