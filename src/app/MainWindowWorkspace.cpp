...
CanvasTabPage *MainWindow::createTabPage(CanvasPane *targetPane)
{
    ...
    // スタートページの操作は、どのタブのスタートページかを問わず「そのタブ自身」をキャンバスへ変える(新しいタブを作るのではない)。
    ...
    // 透過タブの穴の位置・大きさ・表示状態が変わったらウィンドウのマスクを貼り直す(ウィンドウのリサイズ、分割・スプリッターのドラッグ、タブ切替がここへ集まる)。
...
CanvasWidget *MainWindow::ensureCanvas(CanvasTabPage *page)
{
    ...
    // このウィンドウで最初のキャンバスを作る前に、今のドック配置を控えておく(使い道は下のCanvasWidget::initializedハンドラのコメント)。
    ...
    gl->document().setMaxUndo(SettingsDialog::loadValues().undoHistoryLimit);
    // この時点ではまだスタートページを表示したままにしておく。
    page->setCanvas(gl);
    updateTabLabel(page);
    // このキャンバスのGL初期化が終わった時点でもう一度塗り直す。
...
void MainWindow::restoreDockLayoutAfterFirstCanvas(const QByteArray &before, int attempt)
{
    ...
    // GL初期化の直後から 0 / 250 / 600 / 1000ms の4回見る。
...
void MainWindow::bindCanvasWidget(CanvasWidget *gl)
{
    ...
    // layersChanged()はストローク確定の連打や不透明度スライダーのドラッグ等で短時間に連続発火しうる。
    ...
    glWidgetConnections_ << connect(glWidget, &CanvasWidget::activeToolChanged, this, [this](ToolType tool) {
        ...
        toolPropertyDock->setCurrentTool(tool);
    ...
    toolPropertyDock->setCanvasWidget(glWidget);
...
{
    if (paneTreeRoot_) {
        // 既存のツリーがあれば、通常はactivePane_が常に有効な追加先。
...
void MainWindow::unbindCanvasWidget()
{
    ...
    toolPropertyDock->setCanvasWidget(deckCanvasWidget_);
...
void MainWindow::activateTabPage(CanvasTabPage *page)
{
    ...
    if (gl->isGLReady()) {
        ...
        // 作成直後でまだinitializeGL()が完了していないキャンバスの場合、完了を待ってから接続を張る。
                            ...
                            QTabWidget *sourceTabs, int sourceIndex)
{
    ...
    if (!movedPage) return;
    // 自分自身の(唯一の)タブを自分自身へ分割しようとした場合、分割先が無くなってしまうので何もしない。
    ...
    // 折りたたみ規則: paneの直接の親がすでに同じorientationのQSplitterなら、新規QSplitterで包まずそこへ兄弟として追加するだけにする(無駄な入れ子を作らない)。
    ...
    if (parentSplitter && parentSplitter->orientation() == orientation) {
        ...
        // 必ず「先にpaneをnewSplitterへ移す」→「後からnewSplitterをツリーへ組み込む」の順で行う。
        ...
        if (paneWasRoot) {
            // paneは既に上のaddWidget()でcentralStackから外れている(reparentに伴うChildRemovedでQStackedLayoutが自動的にページを削除する)。
    ...
    // ここでページ(CanvasWidgetを含む)が別ペインの子になる=reparent。
    ...
    // paneTreeRoot_が新しいQSplitterに差し替わっている場合、centralStackへaddWidget()しただけではカレントページにならない(むしろ元のページを外した時点でStartPage側へ切り替わってしまう)ため、
    // 明示的に貼り直す。
    updateCentralPage();
...
void MainWindow::closePaneIfEmpty(CanvasPane *pane)
{
    ...
    if (parentSplitter->count() == 1) {
        ...
        if (parentSplitter == paneTreeRoot_) {
            // remainingをcentralStackへ移すと、その時点でparentSplitterからは自動的に外れる。
    ...
    // スプリッターを畳んでpaneTreeRoot_が差し替わった場合に、centralStackのカレントページを新しいルートへ貼り直す(splitPane()末尾と同じ理由)。
    updateCentralPage();
}

// centralStackの表示ページを現在のペインツリーへ合わせ、キャンバス依存アクションの有効/無効を揃える。
void MainWindow::updateCentralPage()
{
    ...
    updateTabRelatedActionsEnabled();
    updateWindowMask();
    // ペインの分割・畳み・タブの増減で構成が変わったぶんを塗り直す。
    scheduleNativeRepaint();
}
...
{
    // index == -1(そのペインのタブが0枚になった)ならwidget()がnullptrになり、activateTabPage()側で無視される。
...
void MainWindow::onTabCloseRequested(CanvasPane *pane, int index)
{
    ...
    if (gl) {
        // ここでdeleteLater()して実際にQOpenGLWidgetを破棄すると、同じウィンドウ内の他のQOpenGLWidget(=他のタブ)と暗黙に共有しているGLコンテキストグループが道連れで壊れ、
        // 他のタブの描画がクラッシュすることがある(特に最初に作られたタブを閉じた場合)。
        page->hide();
    } else {
...
// デバッグ用: 保存済みの全設定(QSettings、ウィンドウ配置・ショートカット・環境設定・最近使ったファイル一覧など)をリセットしてアプリを終了する。
