...
private:
    // タブ1枚 = CanvasTabPage 1枚。CanvasTabPageはスタートページとキャンバス(CanvasWidget)を
    // 内部に持ち、どちらか一方を表示する。タブは必ずスタートページとして生まれ、そこで
    // 作品を選ぶ/新規作成するとキャンバスへ切り替わる。CanvasDocumentはCanvasWidget自身が
    ...
    // 現在アクティブなタブのCanvasWidget。アクティブなタブがまだスタートページの間はnullptr。
    ...
    // Dock類(ToolDock等)のコンストラクタはCanvasWidget*を要求するため、タブが1枚も
    ...
    // 新しいタブのCanvasWidgetに対して張り直す(bindCanvasWidget()参照)。
    ...
    // 長押し開始時点でアクティブだったツール。CanvasWidget::previousTool(スポイトの
    ...
    // すでにキャンバスなら既存のCanvasWidgetをそのまま返す。
    ...
    void addNewTab();
    // glWidgetメンバを指定のCanvasWidgetに切り替え、そのCanvasWidgetの各シグナルをMainWindow/
    // 各Dockへ接続し(古い接続はglWidgetConnections_経由で解除済みである前提)、
    // 各Dockのretarget(setCanvasWidget)とrefresh()を行う。タブ作成時・タブ切替時の両方から呼ぶ。
    void bindCanvasWidget(CanvasWidget *gl);
    // 現在のCanvasWidgetへの接続を切ってglWidgetをnullptrに戻し、各Dockをダミー
    // (deckCanvasWidget_)へretargetする。アクティブなタブがスタートページになったとき用。
    void unbindCanvasWidget();
    // 操作権をpageへ移す単一の入口。所属ペイン(tabOwnerPane_)を特定してそのタブを
    // カレントにし、アクティブペインの見た目(枠線)を付け替えたうえで、pageが
    // キャンバスならbindCanvasWidget()、まだスタートページならunbindCanvasWidget()する。
