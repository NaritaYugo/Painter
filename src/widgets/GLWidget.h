...
#include "backend/CanvasCompositor.h"
#include "backend/LayerSliceAllocator.h"
...
class GLWidget : public QOpenGLWidget, protected QOpenGLFunctions_4_3_Core, public CanvasActionHost
{
    Q_OBJECT
...
public:
    ...
    // GLWidget自身と各Toolインスタンス(penTool_/eraserTool_/fillTool_)で共有する。
    explicit GLWidget(ToolConfig *toolCfg, QWidget *parent = nullptr);
    ~GLWidget() override;
    ...
    // GL資源の後処理が必要なためGLWidgetにしか置けないものだけ。
    ...
    // ------------------------------------------------------------------
    // CanvasDocument (GLWidgetが所有する。1タブ = 1GLWidget = 1CanvasDocument)
    // ------------------------------------------------------------------
    // スライス確保はGLテクスチャ操作を伴うため、GLWidget(のLayerSliceAllocator)
...
public:
    // ---- CanvasActionHost 実装(色調整/フィルター等のアクションが GLWidget の内部に
...
private:
    ...
    // (GLWidget.hのlastTabletPressure_/lastTabletEventClock_コメント参照)。
    ...
    // 画面に残ることがあるため、その場合だけ回避策を行う(GLWidget.cppの
    // 実装コメント参照)。
    ...
    // カーソルの識別子(GLWidget.cppの実装コメント参照)。
    ...
    void cancelNonControllerActions();  // まだ GLWidget 側に残る変形/キャンバス/レイヤー編集アクションを cancel
    ...
    // (GLWidget::paintSelectionOutlineのコメント参照)。選択範囲・ビュー変換・
    ...
    // (実行中フラグ・バックアップ・パネル・位置調整とも)。GLWidget に残るのは、
    ...
    // 破壊的フィルター版が使う filterScratchTex_ (GLWidget.h参照)とはあえて共用
    ...
    // (GLWidget::undo()/redo()が、これから復元するエントリが実際に触れるタイルだけを
