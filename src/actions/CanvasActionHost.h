...
// GLWidget の内部実装に直接依存せずに必要な資源だけを取得するための狭い
// インターフェース。GLWidget がこれを実装する。
//
...
// これらを介することで、アクションは GLWidget の巨大なヘッダを include せずに済み、
// GLWidget 側もアクションの中身を知らなくてよくなる。
// ---------------------------------------------------------------------------
...
public:
    ...
    virtual void hostMakeCurrent()  = 0; // GLWidget::makeCurrent()
    virtual void hostUpdate()       = 0; // GLWidget::update()
    virtual void hostUpdateCursor() = 0; // GLWidget::updateCursor()

    // レイヤー編集系アクション(調整/単色/テキスト)専用。
    // テキストボックスのラスタライズはタイル書き込み・GL資源再確保を伴う低レベル処理
    // (TILE_SIZE・スライス・growLayerBounds等)のため、GLWidget側に残したまま委譲する。
    ...
    // オフスクリーンで作り置きしてある合成結果(GLWidget::rebuildFilterChain)を
