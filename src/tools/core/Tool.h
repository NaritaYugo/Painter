...
// CanvasWidget はイベントを activeTool_ に委譲するだけになる。
// ---------------------------------------------------------------------------
class Tool
{
public:
    ...
    // 現在ストローク中か(CanvasWidget::mouseMoveEvent/mouseReleaseEventのガードに使う)
    virtual bool isActive() const { return false; }

    // ドラッグ中、CanvasWidget側の「一定間隔で同期repaintする」処理を回す必要があるか。
    ...
    // タブレットの傾きとペン軸まわりの回転。筆圧と同じく、CanvasWidget が
    ...
    // マウスカーソルの見た目。std::nullopt を返すと、呼び出し側(CanvasWidget)が
    ...
    // フレームレート律速バッチ用のフック。CanvasWidget::paintGL()が毎フレームの描画
    ...
    //
    // CanvasWidget の取りこぼし用タイマー(inputFlushTimer_)は、これが true のときだけ
    ...
    //
    // CanvasWidget はこの間、描画中にしか意味の無い処理を省いて1フレームを軽くする
    // (CanvasWidget::paintGL のオーバーレイのコメント参照)。
    virtual bool transformsViewWhileActive() const { return false; }

    // フレームレート律速バッチの「入力を貯めてから実際にGPUへ描くまで」許容する最大
    // 遅延(ミリ秒)。CanvasWidget::mouseMoveEvent はこの間隔を超えたときだけ
