...
// ToolContext
// ---------------------------------------------------------------------------
// CanvasWidget が保持する共有GL資源・ドキュメントへの「参照だけ」を集めた構造体。
// 各 Tool はこれを使って、共有のテクスチャ/バッファへ直接書き込む。
// 所有権は一切持たない（CanvasWidget が生成・破棄する）。
// ---------------------------------------------------------------------------
struct ToolContext {
    ...
    // 1デバイスpxに一致し、等倍表示が実画素どおりになる。詳細はCanvasWidget.hの
    ...
    // へ切り替える(CanvasWidgetがマスクサムネイルのクリックに応じて更新する)。
    ...
    // 変わる設定のときだけ使う(CanvasWidget::ensureStrokeColorTexへの委譲)。
    ...
    // ペンストロークのバッチスタンプ用SSBO(所有はCanvasWidget)。stroke.comp の
    ...
    // 選択範囲マスク(所有はCanvasWidget)。canvasW x canvasH の R8。
    ...
    // 作業用スナップショットテクスチャ(所有はCanvasWidget、両アクションで共用)。
    ...
    // キャンバスサイズ変更確定時に呼ぶ(CanvasSizeTool専用)。CanvasWidget::resizeCanvasKeepingContentへの委譲。
    ...
    // 覆うように拡張する(既存矩形との和集合。CanvasWidget::growLayerBoundsToCoverCanvasTilesへの委譲)。
    ...
    // 指定サイズで確保し、そのテクスチャIDを返す(CanvasWidget::ensureFilterScratchへの委譲)。
    // 読み込み元(fullLayerTex)と書き込み先(transformSrcTex)のどちらも潰せないため、
    // パスの間に挟むもう1枚が要る。詳細は CanvasWidget.h の filterScratchTex_ のコメント参照。
    std::function<GLuint(int w, int h)> ensureFilterScratch;

    // 画像解像度変更確定時に呼ぶ(ImageResolutionTool専用)。CanvasWidget::resampleCanvasResolutionへの委譲。
    // 見た目(縦横比・表示上のサイズ)を変えずに、全レイヤーの内容を新しい解像度(newW,newH)へ
    // 拡大縮小(リサンプル)する。
    std::function<bool(int newW, int newH)> resampleCanvasResolution;
 
    // レイヤー情報をGPUに渡すためのSSBO(所有はCanvasWidget)。すべてフラットな
    ...
    // CanvasWidget::startOrEditTextBoxへの委譲(パネルを開き、確定/キャンセルは
    // パネルのシグナル経由でCanvasWidget側が処理する)。
    std::function<void(int boxIndex)> startOrEditTextBox;

    // テキストボックスをキャンバス上で直接ドラッグ移動/拡縮/回転している間、
    // パネルを介さずにレイヤーの実ピクセルへ焼き込みたい場合に呼ぶ
    // (CanvasWidget::scheduleTextRasterizeへの委譲。デバウンスされる)。
    ...
    // 元の CanvasWidget::toPreMulColor(activeSettings().rawColor, activeSettings().opacity) と同じ
    ...
    // (CanvasWidget::widgetToPixel を呼ぶための関数ポインタ的委譲)
    ...
    // CanvasWidget側のhasSelection状態とselectionChangedシグナルの発行に使う。
    ...
    // CanvasWidget側のマーチングアンツ用キャッシュ(paintSelectionOutline)を無効化する。
    ...
    // キャンバス領域(キャンバスpx、両端含む)をCanvasWidgetへ報告する。CanvasWidget::paintGL()は
