...
void BrightnessContrastTool::initialize(QOpenGLContext *ctx)
{
    Q_UNUSED(ctx);
    initializeOpenGLFunctions();
}

void BrightnessContrastTool::activate(ToolContext &ctx)
{
    Q_UNUSED(ctx);
    brightness_ = contrast_ = 0;
    engaged_ = true;
}

void BrightnessContrastTool::deactivate()
{
    engaged_ = false;
    brightness_ = contrast_ = 0;
}

void BrightnessContrastTool::confirm(ToolContext &ctx)
{
    if (!engaged_) return;
...
void BrightnessContrastTool::applyAdjustment(ToolContext &ctx)
{
    if (brightness_ == 0 && contrast_ == 0) return;
    ...
    // 2. brightnessContrast.comp で fullLayerTex に直接(読み書き両用で)焼き込む
    ...
    ctx.computeBrightnessContrastProgram->bind();
    ctx.computeBrightnessContrastProgram->setUniformValue("uBrightnessShift", brightness_ / 200.0f);
    ctx.computeBrightnessContrastProgram->setUniformValue("uContrastFactor",  1.0f + contrast_ / 100.0f);
    glDispatchCompute((canvasW + 15) / 16, (canvasH + 15) / 16, 1);
    glMemoryBarrier(GL_SHADER_IMAGE_ACCESS_BARRIER_BIT);
    ctx.computeBrightnessContrastProgram->release();
