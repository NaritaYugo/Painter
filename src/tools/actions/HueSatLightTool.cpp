...
void HueSatLightTool::initialize(QOpenGLContext *ctx)
{
    Q_UNUSED(ctx);
    initializeOpenGLFunctions();
}

void HueSatLightTool::activate(ToolContext &ctx)
{
    Q_UNUSED(ctx);
    hue_ = sat_ = light_ = 0;
    engaged_ = true;
...
void HueSatLightTool::deactivate()
{
    engaged_ = false;
    hue_ = sat_ = light_ = 0;
}

void HueSatLightTool::confirm(ToolContext &ctx)
{
    if (!engaged_) return;
...
void HueSatLightTool::applyAdjustment(ToolContext &ctx)
{
    if (hue_ == 0 && sat_ == 0 && light_ == 0) return;
    ...
    // 2. hueSatLight.comp で fullLayerTex に直接(読み書き両用で)焼き込む。
    //    ピクセル位置を動かさない演算なので、変形系アクションと違い
    //    読み込み元と書き込み先を分離するスナップショットは不要。
    ...
    ctx.computeHueSatLightProgram->bind();
    ctx.computeHueSatLightProgram->setUniformValue("uHueShift",   (float)hue_);
    ctx.computeHueSatLightProgram->setUniformValue("uSatShift",   sat_   / 100.0f);
    ctx.computeHueSatLightProgram->setUniformValue("uLightShift", light_ / 100.0f);
    glDispatchCompute((canvasW + 15) / 16, (canvasH + 15) / 16, 1);
    glMemoryBarrier(GL_SHADER_IMAGE_ACCESS_BARRIER_BIT);
    ctx.computeHueSatLightProgram->release();
